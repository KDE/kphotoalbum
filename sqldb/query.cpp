...
{
    QString prefix = QString::fromLatin1( "SELECT q0.fileName FROM " );
    QString query;
    QString filesMatch;
    int idx = 0;
    for( QValueList<OptionMatcher*>::ConstIterator it = matcher->_elements.begin(); it != matcher->_elements.end(); ++it, ++idx ) {
        ...
        if ( (valueMatcher = dynamic_cast<OptionValueMatcher*>( *it ) ) )
            query += buildValue( valueMatcher, idx, false );
            ...
            if ( !childValue ) {
                ...
                query += buildValue( childValue, idx, true );
        }
        else
            qFatal("NYI: Matching in OptionAndMatcher" );
    }
    ...
    return QString::fromLatin1( "%1 WHERE %2 %3 %4").arg( prefix ).arg( filesMatch ).arg( possibleAnd ).arg ( query );
}

QString SQLDB::buildValue( OptionValueMatcher* matcher, int idx, bool negate )
{
    QStringList values;
    ...
    for( QStringList::Iterator it = values.begin(); it != values.end(); ++it ) {
        ...
        expression += QString::fromLatin1( "q%1.value = \"%2\"" ).arg( idx ).arg( *it );
    }
    if ( negate )
    ...
    return QString::fromLatin1( "q%1.category = \"%2\" and %3 " )
        .arg(idx).arg( matcher->_category ).arg( expression );
}
